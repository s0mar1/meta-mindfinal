...
export const analyzeAndCacheDeckTiers = async () => { //
  ...
  try {
    ...
    /* ───────────────── ② DB 저장 ───────────────── */
    // const cdnBaseUrl = 'https://raw.communitydragon.org/latest/game/'; // 이 라인 삭제
        ...
        .sort((a,b)=>b[1].count-a[1].count).slice(0,8).map(([apiName,u])=>{ //
          ...
          const recommendedItems = Object.entries(itemCounts) //
            .sort((a,b)=>b[1]-a[1]).slice(0,3).map(([itemApi])=>{ //
              ...
              return { //
                name     : it?.name || '', //
                // image_url: it?.icon ? `${cdnBaseUrl}${it.icon.toLowerCase().replace('.tex','.png')}` : null, // 이 라인 변경
                image_url: it?.icon || null, // tftDataService에서 이미 가공된 icon URL 사용
              };
            });
          return { //
            name     : champInfo?.name || 'Unknown', //
            apiName  : champInfo?.apiName, //
            // image_url: champInfo?.tileIcon // 이 라인 변경
            //   ? `${cdnBaseUrl}${champInfo.tileIcon.toLowerCase().replace('.tex','.png')}` // 이 라인 변경
            //   : null, // 이 라인 변경
            image_url: champInfo?.tileIcon || champInfo?.icon || null, // tftDataService에서 이미 가공된 tileIcon 또는 icon URL 사용
            cost : u.cost, //
            recommendedItems, //
          };
        });
